...
def test_udp_listen_add_duplicate(projector_manager_memdb, caplog):
    """
    ...
    projector_manager_memdb.pjlink_udp = port_list

    # WHEN: udp_listen_add is called with duplicate port number
    caplog.clear()
    projector_manager_memdb.udp_listen_add(port=10)

    # THEN: Verify log entry and registry entry not called
    assert projector_manager_memdb.pjlink_udp == port_list, "Invalid ports in list"
...
def test_udp_listen_add_new(mock_registry, mock_udp, projector_manager_memdb, caplog):
    """
    ...
    projector_manager_memdb.pjlink_udp = {10: 'port1'}
    # WHEN: Adding new listener
    caplog.clear()
    projector_manager_memdb.udp_listen_add(port=20)
    # THEN: Appropriate listener and log entries
    assert 20 in projector_manager_memdb.pjlink_udp, "Port not added"
    assert 2 == len(projector_manager_memdb.pjlink_udp), "Invalid ports in list"
    assert type(projector_manager_memdb.pjlink_udp[20]) == FakePJLinkUDP, \
        'PJLinkUDP instance should have been added'
    assert mocked_registry.execute.has_call('udp_broadcast_add', port=20)
    assert caplog.messages == log_entries, 'Invalid log entries'


def test_udp_listen_delete_missing(projector_manager_memdb, caplog):
    """
    ...
    projector_manager_memdb.udp_listen_delete(port=20)
    # THEN: Log missing port and exit method
    assert projector_manager_memdb.pjlink_udp == {10: 'port1'}, "Invalid ports in list"
    assert caplog.messages == log_entries, "Invalid log entries"


@patch('openlp.core.projectors.manager.Registry')
def test_udp_listen_delete_single(mock_registry, projector_manager_memdb, caplog):
    """
    ...
    port_list = {20: 'port2'}
    projector_manager_memdb.pjlink_udp = {10: 'port1', **port_list}
    projector_manager_memdb.projector_list = [FakeProjector(port=20)]
    # WHEN: deleting a listener
    caplog.clear()
    projector_manager_memdb.udp_listen_delete(port=10)
...
def test_udp_listen_delete_skip(projector_manager_memdb, caplog):
    """
    ...
    port_list = {10: 'port1', 20: 'port2'}
    projector_manager_memdb.pjlink_udp = port_list
    projector_manager_memdb.projector_list = [FakeProjector(port=10),
