...
from openlp.core.projectors.constants import E_AUTHENTICATION, PJLINK_ERRORS, S_DATA_OK,\
    STATUS_MSG
...
def test_data_ok(pjlink, caplog):
    ...
    # THEN: Appropriate log entries should have been made and methods called/not called
    assert chk is test_return, '"{test_data}" reply should have returned {STATUS_CODE[test_return]}'
    assert caplog.record_tuples == logs, 'Log entries mismatch'


def test_data_error(pjlink, caplog):
    """
    Test data == PJLINK_ERROR
    """
    # GIVEN: Test setup
    caplog.set_level(logging.DEBUG)
    ...
    # THEN: Appropriate log entries should have been made and methods called/not called
    assert chk is test_return, f'"{PJLINK_ERRORS[test_error]}" reply should have returned {test_error}'
    assert caplog.record_tuples == logs, 'Log entries mismatch'
